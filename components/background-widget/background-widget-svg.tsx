 ...
 *
 * All colors are fully configurable via props, making this component
 * easy to adapt to any light/dark theme or custom palette.
 *
 * @example
 ...
 *
 * Extends all standard SVG props (width, height, preserveAspectRatio, etc.)
 * with optional color overrides for each visual element.
 */
 ...
 * BackgroundSvg
 *
 * A purely presentational SVG component that renders four decorative
 * shapes on a transparent canvas (1874 × 960 internal units).
 *
 * This component is intentionally stateless and has no side-effects;
 * animation (rotation, etc.) is applied by the parent wrapper via
 * React Native's `Animated.View` or `react-native-reanimated`.
 *
 ...
 * @param ...props - Any additional props forwarded to the underlying `<Svg>` element.
 *
 * @returns A React Native SVG element with four decorative shapes.
 */
const BackgroundSvg = ({
  ...
  >
    ...
    {/* Clip mask that confines all shapes to the 1874×960 canvas */}
    <Defs>
      <ClipPath id="a">
        <Path fill="#fff" d="M0 0h1874v960H0z" />
      </ClipPath>
    </Defs>
  </Svg>
);
